...
class DynamicPrompt
  ...
  def get_activeRetrieve current active prompt from Redis#     @storage.get_active
  end
  ...
  def revertRestore prompt to original (canonical) version#     backup = @storage.get_backup
  ...
  def diffShow changes between active and original prompt#     original = @storage.get_backup
  ...
  def history(limit: 10)Get modification history (audit trail)#     @logger.get_history(limit)
  end
  
  def clear!Clear all stored prompts and history (dangerous!)#     @storage.clear_all!
    @logger.clear_all!
  ...
  def load_from_file(path)Load prompt from file system#     File.read(path, encoding: 'UTF-8')
  end
  
  def load_from_url(url)Load prompt from HTTP(S) URL#     uri = URI.parse(url)
